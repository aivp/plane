...
export const useCoreOAuthConfig = (oauthActionText: string): TOAuthConfigs => {
  ...
  // derived values
  const getOAuthText = (provider: string) =>
    t("auth.common.oauth_with_provider", { action: oauthActionText, provider });
  ...
  const oAuthOptions: TOAuthOption[] = [
    ...
    {
      id: "lark",
      text: getOAuthText("飞书"),
      icon: <img src={feishuLogo} height={18} width={18} alt="飞书 Logo" />,
      onClick: () => {
        window.location.assign(`${API_BASE_URL}/auth/spaces/lark/${next_path ? `?next_path=${next_path}` : ``}`);
      },
